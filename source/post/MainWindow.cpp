...
OutputWidget::OutputWidget(const OutputData& output)
{
    ...
    if(enable_statics) {
        stack->addWidget(new StaticOutputWidget(output));
    }
    if(enable_dynamics) {
        stack->addWidget(new DynamicOutputWidget(output));
    }
...
StaticOutputWidget::StaticOutputWidget(const OutputData& output)
{
    ...
    for(int i = 0; i < output.statics.max_stress_index.size(); ++i) {
        numbers->addValue(QString::fromStdString(output.setup.limb_properties.layers[i].name + " [Pa]"), output.statics.max_stress_value[i]);
    }
    ...
    auto plot_stress = new StressPlot(output.setup.limb_properties, output.statics.states);
...
DynamicOutputWidget::DynamicOutputWidget(const OutputData& output)
{
    auto numbers = new NumberGrid();
    numbers->addColumn();
    numbers->addGroup("Performance");
    ...
    for (int i = 0; i < output.dynamics.max_stress_value.size(); ++i) {
        numbers->addValue(QString::fromStdString(output.setup.limb_properties.layers[i].name + " [Pa]"), output.dynamics.max_stress_value[i]);
    }
    numbers->addGroup("Maximum absolute forces");
    ...
    auto plot_stress = new StressPlot(output.setup.limb_properties, output.dynamics.states);
