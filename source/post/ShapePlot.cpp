...
void ShapePlot::plotLimbOutline(QCPCurve* left, QCPCurve* right, const VectorXd& x, const VectorXd& y, const VectorXd& phi)
{
    ...
    {
        double xi = x[i] - 0.0*limb.height[i]*sin(phi[i]);
        double yi = y[i] + 0.0*limb.height[i]*cos(phi[i]);
        left->addData(-xi, yi);
        right->addData(xi, yi);
    }
    ...
    {
        double xi = x[i] + 1.0*limb.height[i]*sin(phi[i]);
        double yi = y[i] - 1.0*limb.height[i]*cos(phi[i]);
        left->addData(-xi, yi);
        right->addData(xi, yi);
    }
}
