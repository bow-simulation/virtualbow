    ...
    : tabs(new QTabWidget())
{
    ...
    numbers->addValue("Final draw force", data.statics.final_draw_force, UnitSystem::force);
    numbers->addValue("Drawing work", data.statics.drawing_work, UnitSystem::energy);
    numbers->addValue("Energy storage factor", data.statics.energy_storage_factor, UnitSystem::ratio);
    numbers->addGroup("Properties");
    numbers->addValue("Limb mass", data.setup.limb_mass, UnitSystem::mass);
    numbers->addValue("String mass", data.setup.string_mass, UnitSystem::mass);
    numbers->addValue("String length", data.setup.string_length, UnitSystem::mass);

    numbers->addColumn();
    numbers->addGroup("Minimum stress by layer");
    for(size_t i = 0; i < data.statics.min_stress_index.size(); ++i) {
        numbers->addValue(QString::fromStdString(data.setup.limb_properties.layers[i].name), data.statics.min_stress_value[i], UnitSystem::stress);
    }
    numbers->addGroup("Maximum stress by layer");
    for(size_t i = 0; i < data.statics.max_stress_index.size(); ++i) {
        numbers->addValue(QString::fromStdString(data.setup.limb_properties.layers[i].name), data.statics.max_stress_value[i], UnitSystem::stress);
    }

    numbers->addColumn();
    numbers->addGroup("Maximum absolute forces");
    numbers->addValue("Draw force", data.statics.states.draw_force[data.statics.max_draw_force_index], UnitSystem::force);
    numbers->addValue("Grip force", data.statics.states.grip_force[data.statics.max_grip_force_index], UnitSystem::force);
    numbers->addValue("String force (total)", data.statics.states.string_force[data.statics.max_string_force_index], UnitSystem::force);
    numbers->addValue("String force (strand)", data.statics.states.strand_force[data.statics.max_string_force_index],  UnitSystem::force);
    ...
    auto plot_energy = new EnergyPlot(data.statics.states, data.statics.states.draw_length, "Draw length", UnitSystem::length, UnitSystem::energy);
    auto plot_combo = new ComboPlot();
    plot_combo->addData("Draw length", data.statics.states.draw_length, UnitSystem::length);
    plot_combo->addData("Draw force", data.statics.states.draw_force, UnitSystem::force);
    plot_combo->addData("String force (total)", data.statics.states.string_force, UnitSystem::force);
    plot_combo->addData("String force (strand)", data.statics.states.strand_force, UnitSystem::force);
    plot_combo->addData("Grip force", data.statics.states.grip_force, UnitSystem::force);
    plot_combo->addData("Pot. energy limbs", data.statics.states.e_pot_limbs, UnitSystem::energy);
    plot_combo->addData("Pot. energy string", data.statics.states.e_pot_string, UnitSystem::energy);
    ...
    auto slider = new Slider(data.statics.states.draw_length, "Draw length", UnitSystem::length);
    ...
    : tabs(new QTabWidget())
{
    ...
    numbers->addColumn();
    numbers->addGroup("Performance");
    numbers->addValue("Final arrow velocity", data.dynamics.final_vel_arrow, UnitSystem::velocity);
    numbers->addValue("Degree of efficiency", data.dynamics.efficiency, UnitSystem::ratio);
    numbers->addGroup("Energy at arrow departure");
    numbers->addValue("Kinetic energy arrow", data.dynamics.final_e_kin_arrow, UnitSystem::energy);
    numbers->addValue("Kinetic energy limbs", data.dynamics.final_e_kin_limbs, UnitSystem::energy);
    numbers->addValue("Kinetic energy string", data.dynamics.final_e_kin_string, UnitSystem::energy);
    numbers->addColumn();
    numbers->addGroup("Maximum absolute stresses");
    for (size_t i = 0; i < data.dynamics.max_stress_value.size(); ++i) {
        numbers->addValue(QString::fromStdString(data.setup.limb_properties.layers[i].name), data.dynamics.max_stress_value[i], UnitSystem::stress);
    }
    numbers->addGroup("Maximum absolute forces");
    numbers->addValue("String force (total)", data.dynamics.states.string_force[data.dynamics.max_string_force_index], UnitSystem::force);
    numbers->addValue("String force (strand)", data.dynamics.states.strand_force[data.dynamics.max_string_force_index], UnitSystem::force);
    numbers->addValue("Grip force", data.dynamics.states.grip_force[data.dynamics.max_grip_force_index], UnitSystem::force);
    ...
    auto plot_energy = new EnergyPlot(data.dynamics.states, data.dynamics.states.time, "Time", UnitSystem::time, UnitSystem::energy);
    auto plot_combo = new ComboPlot();
    plot_combo->addData("Time", data.dynamics.states.time, UnitSystem::time);
    plot_combo->addData("Arrow position", data.dynamics.states.pos_arrow, UnitSystem::position);
    plot_combo->addData("Arrow velocity", data.dynamics.states.vel_arrow, UnitSystem::velocity);
    plot_combo->addData("Arrow acceleration", data.dynamics.states.acc_arrow, UnitSystem::acceleration);
    plot_combo->addData("String force (total)", data.dynamics.states.string_force, UnitSystem::force);
    plot_combo->addData("String force (strand)", data.dynamics.states.strand_force, UnitSystem::force);
    plot_combo->addData("Grip force", data.dynamics.states.grip_force, UnitSystem::force);
    plot_combo->addData("Pot. energy limbs", data.dynamics.states.e_pot_limbs, UnitSystem::energy);
    plot_combo->addData("Kin. energy limbs", data.dynamics.states.e_kin_limbs, UnitSystem::energy);
    plot_combo->addData("Pot. energy string", data.dynamics.states.e_pot_string, UnitSystem::energy);
    plot_combo->addData("Kin. energy string", data.dynamics.states.e_kin_string, UnitSystem::energy);
    plot_combo->addData("Kin. energy arrow", data.dynamics.states.e_kin_arrow, UnitSystem::energy);
    plot_combo->setCombination(0, 1);
    ...
    auto slider = new Slider(data.dynamics.states.time, "Time", UnitSystem::time);
