...
int LimbSource::RequestData(vtkInformation* request, vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
    ...
    for(size_t i = 0; i < n_sections; ++i)
    {
        ...
        for(size_t j = 0; j < n_layers; ++j)
        {
            ...
            Vector<3> pl = center - 0.5*normal_w*limb.width[i] + normal_h*limb.layers[j].y_back[i];
            indices_l1[j] = points->InsertNextPoint(pl.data());

            Vector<3> pr = center + 0.5*normal_w*limb.width[i] + normal_h*limb.layers[j].y_back[i];
            indices_r1[j] = points->InsertNextPoint(pr.data());

            if(j == n_layers-1)
            {
                Vector<3> pl = center - 0.5*normal_w*limb.width[i] + normal_h*limb.layers[j].y_belly[i];
                indices_l1[j+1] = points->InsertNextPoint(pl.data());

                Vector<3> pr = center + 0.5*normal_w*limb.width[i] + normal_h*limb.layers[j].y_belly[i];
