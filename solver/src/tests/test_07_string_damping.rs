...
fn verify_analytic_damping_ratio() {
    ...
    for (i, &s) in lengths.iter().enumerate() {
        ...
        nodes.push(system.create_point_node(&vector![s, 0.0], constraints));
    }
    ...
    for (i, mode) in modes.iter().enumerate() {
        let k = i + 1;
        let omega_ref = PI*((2*k - 1) as f64)/(2.0*L)*f64::sqrt(EA/rhoA);
        let zeta_ref = PI*((2*k - 1) as f64)/(4.0*L)*etaA/f64::sqrt(rhoA*EA);
