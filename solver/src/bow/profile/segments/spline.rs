...
impl PlanarCurve for SplineSegment {
    ...
    fn deriv1(&self, s: f64) -> SVector<f64, 2> {
        ...
        // We have the components x = x(t(s)) and y = y(t(s)) of the curve, therefor the derivative wrt. arc lengh is
        //
        // dx/ds = dx/dt * dt/ds
        // dy/ds = dx/dt * dt/ds
        //
        // We could get dt/ds directly from the parameter function t(s), but this is inaccurate.
        // Instead we use the exaxct expression
        //
        ...
        let dxdt = self.spline_x.deriv1(t, Extrapolation::Cubic);
        let dydt = self.spline_y.deriv1(t, Extrapolation::Cubic);
        let dtds = 1.0/f64::hypot(dxdt, dydt);
    ...
    fn deriv2(&self, s: f64) -> SVector<f64, 2> {
        ...
        // For the first derivative we used the expressions
        //
        // dx/ds = dx/dt * dt/ds
        // dy/ds = dx/dt * dt/ds
        //
        // Deriving one more time, using the product rule, leads to
        //
        // d²x/ds² = d/ds(dx/dt)*dt/ds + dx/dt*d/ds(dt/ds) = d²x/dt²*(dt/ds)² + dx/dt*d²t/ds²
        // d²y/ds² = d/ds(dx/dt)*dt/ds + dx/dt*d/ds(dt/ds) = d²y/dt²*(dt/ds)² + dy/dt*d²t/ds²
        // d²t/ds² = d/dt(dt/ds)*dt/ds = -(dx/dt*d²x/dt² + dy/dt*d²y/dt²)/((dx/dt)² + (dy/dt)²)²
        ...
        let dtds = 1.0/f64::hypot(dxdt, dydt);
        let dtds2 = -(dxdt*dxdt2 + dydt*dydt2)/(dxdt*dxdt + dydt*dydt).powi(2);

        vector![
            dxdt2*dtds.powi(2) + dxdt*dtds2,
            dydt2*dtds.powi(2) + dydt*dtds2
        ]
    }
}