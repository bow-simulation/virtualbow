    ...
    where F: FnMut(&DVector<f64>, &mut DVector<f64>, &mut DMatrix<f64>)  // x -> f, dfdx
{
    ...
    let epsilon = (x0.len() as f64)*settings.epsilon.powi(2);
    ...
    for i in 1..=settings.max_iter {
        ...
        loop {
            ...
            if f_next.norm_squared() <= norm_current + 2.0*settings.armijo_constant*alpha*f_current.dot(&(&J_current*&delta_x)) {
          ...
          C: FnMut(&DVector<f64>, f64, &mut f64, &mut DVector<f64>, &mut f64),
{
    ...
    let epsilon = ((x0.len() + 1) as f64)*settings.epsilon.powi(2);
    ...
    for i in 1..=settings.max_iter {
        ...
        for _ in 0..1 {
            ...
            println!("Current: {}, Next: {}, Slope: {}", norm_current, norm_next, slope);

            if norm_next <= norm_current + 2.0*settings.armijo_constant*alpha*slope {
                x_current.copy_from(&x_next);
                f_current.copy_from(&f_next);
                λ_current = λ_next;
                c_current = c_next;
