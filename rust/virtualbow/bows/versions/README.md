The `.bow` files in this directory are used for testing compatibility with older model formats.
This is done to ensure that models created with an older version of VirtualBow can still be loaded and produce the same bow as before (but not necessarily the same simulation results).
