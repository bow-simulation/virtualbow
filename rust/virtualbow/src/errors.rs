...
pub enum ModelError {
    ...
    MaterialInvalidColor(String),
    MaterialInvalidDensity(f64),
    MaterialInvalidYoungsModulus(f64),
    MaterialInvalidShearModulus(f64),
    MaterialInvalidTensileStrength(f64),
    MaterialInvalidCompressiveStrength(f64),
    MaterialInvalidSafetyMargin(f64),
    ...
    LayerHeightControlPointsInvalidBoundaryValue(f64, f64),
    LayerHeightControlPointsInvalidInteriorValue(f64, f64),
    LayerHeightControlPointsDiscontinuousBoundary(f64, f64),
...
impl Display for ModelError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ModelError::MaterialInvalidColor(value)         => write!(f, "Material: Color must be a hex color string (starting with #, followed by 6 digits of 0-9, a-f) but actual value is {value}.")?,
            ...
            ModelError::MaterialInvalidTensileStrength(value)      => write!(f, "Material: Tensile strength must be a positive number but actual value is {value}.")?,
            ModelError::MaterialInvalidCompressiveStrength(value)  => write!(f, "Material: Compressive strength must be a positive number but actual value is {value}.")?,
            ModelError::MaterialInvalidSafetyMargin(value)         => write!(f, "Material: Safety margin must be in the range [0, 1] but actual value is {value}.")?,
            ...
            ModelError::LayerHeightControlPointsInvalidBoundaryValue(a, b)  => write!(f, "Height: Boundary control points must be non-negative and finite but actual value found is [{a}, {b}].")?,
            ModelError::LayerHeightControlPointsInvalidInteriorValue(a, b)  => write!(f, "Height: Intermediate control points must be positive and finite but actual value found is [{a}, {b}].")?,
            ModelError::LayerHeightControlPointsDiscontinuousBoundary(a, b) => write!(f, "Height: Boundary control point at relative length {a} must be zero for continuity but actual value is {b}.")?,
