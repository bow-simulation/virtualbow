...
pub use output::*;
use crate::bow::output::versions::BowOutputVersions;
...
mod tests {
    ...
    fn test_load_output() {
        ...
        assert_matches!(BowOutput::load("bows/tests/output/nonexistent.res"), Err(ModelError::OutputLoadFileError(_, _)));

        // Deserialization error due to the file containing invalid messagepack
        assert_matches!(BowOutput::load("bows/tests/output/invalid_msgpack.res"), Err(ModelError::OutputDecodeMsgPackError(_)));

        // Deserialization error due to the file containing valid messagepack but no version entry
        assert_matches!(BowOutput::load("bows/tests/output/version_missing.res"), Err(ModelError::OutputDecodeMsgPackError(_)));

        // Error when loading a result file with a version that is not supported
        assert_matches!(BowOutput::load("bows/tests/output/version_unsupported.res"), Err(ModelError::OutputVersionUnsupported));

        // Error when loading a result file with a version that is not recognized
        assert_matches!(BowOutput::load("bows/tests/output/version_unrecognized.res"), Err(ModelError::OutputVersionUnrecognized));
        ...
        assert_matches!(BowOutput::load("bows/tests/output/version_invalid.res"), Err(ModelError::OutputVersionUnrecognized));

        // Deserialization error due to invalid file contents (valid messagepack and version but invalid structure)
        assert_matches!(BowOutput::load("bows/tests/output/invalid_content.res"), Err(ModelError::OutputDecodeMsgPackError(_)));

        // No error when loading a valid output file
        assert_matches!(BowOutput::load("bows/tests/output/valid_results.res"), Ok(_));
    ...
    fn test_save_output() {
        ...
        assert_matches!(output.save("bows/tests/output/nonexistent/output.res"), Err(ModelError::OutputSaveFileError(_, _)));
    ...
    fn generate_test_files() {
        ...
        File::create("bows/tests/output/invalid_msgpack.res").unwrap();

        // File with valid msgpack but no version entry
        let mut file = File::create("bows/tests/output/version_missing.res").unwrap();
        ...
        let mut file = File::create("bows/tests/output/version_invalid.res").unwrap();
        ...
        let mut file = File::create("bows/tests/output/version_unsupported.res").unwrap();
        ...
        let mut file = File::create("bows/tests/output/version_unrecognized.res").unwrap();
        ...
        let mut file = File::create("bows/tests/output/invalid_content.res").unwrap();
        ...
        let output = BowOutput::default();
        output.save("bows/tests/output/valid_results.res").unwrap();
    }
