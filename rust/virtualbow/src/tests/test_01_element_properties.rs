...
use crate::bow::input::{Arc, Height, Layer, Material, ProfileAlignment, Width};
...
fn beam_element() {
    ...
    let curve = ClothoidSegment::arc(&CurvePoint::zero(), &Arc{length: l, radius: r});
    let section = LayeredCrossSection::new(l, &width, &vec![layer], &vec![material], &ProfileAlignment::SectionCenter).unwrap();
