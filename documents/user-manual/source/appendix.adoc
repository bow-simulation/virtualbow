...
A bending test is a good way to experimentally determine the elastic modulus stem:[E] of a material.
Starting point is a material sample with a constant width stem:[w] and height stem:[h].
...
The useful length stem:[l] of the sample should be much larger than the cross section dimensions such that it can bend enough for easy measuring while still staying in the elastic range of the material.
On the other hand, the sample shouldn't be so long that it bends significantly under its own weight either.
A suggestion would be stem:[l \ge 100 \cdot h].

To perform the test, the sample is either clamped on one end, as shown in <<#img-setup-clamped>>, or placed between two supports as shown in <<#img-setup-three-point>>.
It is then subjected to a force stem:[F] and the resulting deflection stem:[s] is measured.
A good way to apply an accurate force is to use a known weight stem:[m], suspended for example on a thread, and calculate the force as stem:[F = m \cdot g] with stem:[g=9.81\frac{m}{s^2}].
...
[#img-setup-clamped]
.Bending test with a clamped setup
image::images/bending-test/setup-clamped.svg[width=400, pdfwidth=60%]
...
[stem]
++++
s = \frac{Fl^3}{3EI} \quad \Rightarrow \quad E = \frac{Fl^3}{3Is} = \frac{4Fl^3}{wh^3 s}.
++++
...
[#img-setup-three-point]
.Bending test with a three-point setup
image::images/bending-test/setup-three-point.svg[width=400, pdfwidth=60%]

In this setup the sample is placed between two supports with distance stem:[l] and the force is applied in the middle.
The elastic modulus can be calculated as

[stem]
++++
s = \frac{Fl^3}{48EI} \quad \Rightarrow \quad E = \frac{Fl^3}{48Is} = \frac{Fl^3}{4wh^3 s}.
++++


=== Scripting Examples

This is a collection of example scripts in different programming languages that show how to interface with VirtualBow.
Each of them performs the same series of basic tasks:
