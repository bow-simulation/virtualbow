...
The bow's profile is modeled as a cubic spline, where each segment of the curve is given by cubic polynomials
%
\begin{align}
x(t) &= c_{0} + c_{1}\,t + c_{2}\,t^2 + c_{3}\,t^3, \\
y(t) &= c_{4} + c_{5}\,t + c_{6}\,t^2 + c_{7}\,t^3,
\end{align}

with the parameter $t \in [0,\,1]$ and the eight unknown constants $c_{x0},\ldots\,c_{y3}$ that are yet to be determined.
For a curve given in this form, the curvature $\kappa(t)$, tangent angle $\varphi(t)$ and arc length $l(t)$ can be calculated as
%
\begin{align}
\kappa(t) &= \frac{x'(t)\,y''(t) - y'(t)\,x''(t)}{\left(x'^{\,2}(t) + y'^{\,2}(t)\right)^{\nicefrac{3}{2}}}, \\
\varphi(t) &= \arctan2\left(\frac{y'(t)}{x'(t)}\right) + 2\pi\,\mathbb{Z},  \\
l(t) &= \int_{0}^{t} \sqrt{x'^{\,2}(\tau) + y'^{\,2}(\tau)}\ d\tau,
\end{align}

with the first and second derivatives
%
\begin{align}
x'(t) &= c_{1} + 2\,c_{2}\,t + 3\,c_{3}\,t^2,\\
y'(t) &= c_{5} + 2\,c_{6}\,t + 3\,c_{7}\,t^2, \\
\notag \\
x''(t) &= 2\,c_{2} + 6\,c_{3}\,t, \\
y''(t) &= 2\,c_{6} + 6\,c_{7}\,t.
\end{align}

The users are allowed to specify up to four values from the set $\{\Delta\,l$, $\Delta\,\varphi$, $\Delta\,x$ $\Delta\,y\}$ for each segment.
As shown in figure~\ref{fig:model:profile-segments}, $\Delta l$ is the total length of the segment, $\Delta \varphi$ is the total change in angle, and $\Delta x$, $\Delta y$ are the differences between the start- and endpoint of the segment in the respective coordinate.

\begin{figure}[h]
\centering
\includegraphics[width=0.4\textwidth]{figures/model/profile-segment}
\caption{Profile segment with user supplied constraints}
\label{fig:model:profile-segments}
\end{figure}

These values, if specified, impose the following constraints on the segment
...
\notag \\
&\Delta\,l = l(1) - l(0) \\
&\Delta\,\varphi = \int_{0}^{1} \varphi'(t)\ dt = \int_{0}^{1} \frac{x'\,y'' - y'\,x''}{x'^{\,2} + y'^{\,2}}\ dt
\end{align}

Additional constraints are imposed by requiring the resulting curve to be $C^2$ continuous, i.e. continuous up to the second derivative, thus requiring the following conditions to hold at the connection of segments $i$ and $i+1$,
%
\begin{equation}
\begin{aligned}
x_{i}(1) &= x_{i+1}(0), \\
...
One last set of constraint is requires to specify the starting point of the curve,
%
\begin{equation}
x_0(0) = 0, \qquad y_0(0) = 0, \qquad \varphi_0(0) = 0.
\end{equation}

However, even with all those constraints the curve is still not uniquely determined.
For this reason, we define the following smoothness measure, which is the squared curvature integrated over the length of the curve,
%
\begin{align}
E = \int_{l(0)}^{l(1)} \kappa^2(t(l))\,dl = \int_{0}^{1} \kappa^2(t)\cdot l'(t)\ dt = \int_{0}^{1} \frac{(x'\,y'' - y'\,x'')^2}{(x'^{\,2} + y'^{\,2})^{\nicefrac{5}{2}}}\ dt.
\end{align}

The shape of the curve is then found by minimizing the sum $\sum_i E_i$ subject to the constraints stated above.
In our implementation this is done in two passes by  a numerical optimization algorithm.
In the first pass, a valid but suboptimal solution is generated by optimizing each segment separately, one after the other.
Once the shape of a segment is determined, the starting point (position, angle) of the next segment becomes known, and so on.
This suboptimal solution is then used as an initial value for the second pass, which is the final optimization of the complete curve.

\textcolor{red}{Todo: Parameterization by arc length}

In order for the optimization to work reliably, reasonable initial values for the segments have to be chosen.
The derivation of those for all different combinations of constraints is shown in Appendix \ref{chapter:profile-curve-initial-values}.
