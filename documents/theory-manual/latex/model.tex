...
\subsection{Geometric Description of the Limbs}

\textcolor{red}{Todo: Layers, Width, Profile. Euler spirals, monotonic cubic splines. Stress calculation per layer.}

\newpage
\subsection{Contact Handling}

The contact elements calculate contact forces between string nodes and the beam segments in order to prevent the string from just "slipping through" the limb on a recurve bow.
However, testing each possible combination of string node and limb segment for contact would be too wasteful.
In general, contact handling is therefore usually done in two phases: A broad phase and a narrow phase.
The broad phase algorithm's job is to filter out as many non-contacting pairs of objects as possible without doing an exact contact check.
It does this by using crude methods that are fast but not exact.
Checking these remainig pairs in an exact (but also more costly) way is then the job of the following narrow phase.

Up until version 0.7, VirtualBow used a multi-axis implementation of the sweep and prune broad phase algorithm.
This algorithm keeps sorted lists with the lower and upper bounds of all the objects along the coordinate axes.
It then creates and removes contact elements as overlaps between the coordinates occur or disappear during sorting.

\begin{figure}[h]
\centering
\includegraphics[width=0.4\textwidth]{figures/model/contact-handling}
\caption{Mapping of string nodes to limb elements for contact handling}
\label{fig:model:contact-handling}
\end{figure}

Newer versions of VirtualBow use a much more simple method that creates a fixed number of contact elements before the simulation, which was chosen because it greatly reduced the complexity of the whole finite element code and has shown to be only slightly worse than the old sweep and prune implementation in terms of performance.

As shown in figure \ref{fig:model:contact-handling}, each string node has a distance $s$ from the limb tip, measured as arc length.
This length along the string is mapped to an arc length range
%
\begin{align}
s_\mathrm{min} = (1-c)\,s \\
s_\mathrm{max} = (1+c)\,s
...
along the limb, in which the string node can be expected to contact the limb during the simulation.
A contact element is created for the respective string node and every limb segment that intersects this associated range.
The parameter $c < 1$ is an upper bound of how far the string nodes are expected to move up and down the limb.

As an additional condition, the contact element is only created if $s_\mathrm{min} < p\cdot l$, where $l$ is the total length of the limb and $p < 1$ is the fraction of the limb that is expected to contact the string.
This further limits the number of created contact elements, as most bows contact the string only at their outer  $\sim$ 30\% at most.

The parameters $p$ and $c$ can be adjusted by the user.
For both of them, larger values are more conservative and reliable, but also slow down the simulation as they produce more (potentially unnecessary) contact elements.
Setting $p = 0$ allows to completely disable contact handling to get maximum performance on bow models that don't need it.
