...
\textcolor{red}{TODO: Explain choice of boundary conditions}
\begin{figure}[h]
\centering
...
\newpage
\subsection*{Circular string contact}

In this example, an elastic string is placed around a rigid quarter circle as shown in figure \ref{fig:circular-string-contact}.
Its left end is fixed while the free end can move in the $y$ direction.
As the free end of the string is pushed downwards by the force $F$, the string wraps around the arc and lengthens.
Wanted is the relationship between the force $F$ and the length $h$.

\begin{figure}[H]
\centering
\includegraphics[width=0.4\textwidth]{figures/verification/circular-string-contact.jpg}
\caption{String (red) being wrapped around a circular arc (blue)}
\label{fig:circular-string-contact}
\end{figure}

Geometrical condition: The line from the contact point~$P$ in the direction of the tangent~$T$ at that point must connect to point~$H$.
This translates to
...
This are two equations from which the unknown line parameter $\lambda$ and the angle $\alpha$ of the contact point can be calculated.
Use the first equation to express $\lambda$ in terms of $\alpha$:

\begin{equation}
r\,\cos(\alpha) + \lambda r\,\sin(\alpha) = r \quad \Rightarrow \quad \lambda = \frac{1 - \cos(\alpha)}{\sin(\alpha)} = \tan\left(\frac{\alpha}{2}\right)
\end{equation}

Inserting this into the second equation results in the relationship between $h$ and $\alpha$:

\begin{align}
r\underbrace{\left(\sin(\alpha) - \cos(\alpha)\tan\left(\frac{\alpha}{2}\right)\right)}_{\tan\left(\frac{\alpha}{2}\right)} = r - h  \quad \Rightarrow \quad h &= r\left(1 - \tan\left(\frac{\alpha}{2}\right)\right) \\
\Rightarrow \quad \alpha &= 2\arctan\left(1 - \frac{h}{r}\right)
\end{align}

As a next step, we determine the length $l$ of the string as the sum of the circular and the linear sections:

\begin{align}
l &= \left(\frac{\pi}{2} - \alpha\right)r + \lambda r = \left(\frac{\pi}{2} - \alpha + \tan\left(\frac{\alpha}{2}\right)\right)r
\end{align}

The potential energy of the string with stiffness $k = \frac{EA}{l_0}$ and initial length $l_0$ is known as

\begin{equation}
\Pi = \frac{1}{2}k\left(l - l_{0}\right)^2
\end{equation}

Therefore the force $F$ in the direction of the displacement $h$ is

\begin{align}
F &= \frac{\partial \Pi}{\partial h} = \frac{\partial \Pi}{\partial l} \cdot \frac{\partial l}{\partial \alpha} \cdot \frac{\partial \alpha}{\partial h} \\
&= k(l - l_{0}) \cdot \left(\frac{1}{1 + \cos(\alpha)} - 1\right) \cdot \frac{2r^2}{h^2 - 2hr + 2r^2}
\end{align}
...
\captionof{figure}{Static equilibrium of a bow limb without recurve \textcolor{red}{TODO: Replace $x_t$, $y_t$ with the first contact point $x_c$, $y_c$ on a recurve limb}}
...
\begin{equation}
F_d = 2F_s\,\sin(\alpha),\quad \alpha = \arctan\left(\frac{y_c - y_s}{x_c}\right).
\end{equation}
...
The string exerts a force $F_s$ on the limb tip with the two components
...
The forces and moments $N$, $M$, $Q$ of each cross section of the limb must be in equilibrium with this applied force.
If $x_c$ and $y_c$ is the point where the string begins to contact the bow \textcolor{red}{TODO: add this to the drawing above}, the cross section forces can be calculated as

\begin{align}
M(s) &= F_y(x_c - x(s)) - F_x(y_c - y) \\
N(s) &= F_x\cos(\varphi(s)) + F_y\sin(\varphi(s)) \\
Q(s) &= F_y\cos(\varphi(s)) - F_x\sin(\varphi(s))
\end{align}

Additionally, the cross section forces must also be in balance with the stresses within the section.
This condition is expressed as

\begin{align}
N(s) &= \phantom{-}\int_A \sigma(x,\,y)\,dA \\
Q(s) &= \phantom{-}\int_A \tau(x,\,y)\,dA \\
M(s) &= -\int_A y\sigma(x,\,y)\,dA
\end{align}

and can be checked in tests by numerical integration.
...
The draw force must be equal to the derivative of the potential energy with respect to the draw length.
...
A pendulum tha moves under the influence of gravity is a commonly used example for a simple nonlinear dynamic system.
We first introduce the functions $\mathrm{sn}$, $\mathrm{cn}$ and $\mathrm{dn}$ as the Jacobi elliptic sine, cosine and delta amplitude and $\mathrm{K}$ as the elliptic integral of the first kind.
The analytical solution for the motion of a nonlinear pendulum with initial angle $\varphi(0) = \varphi_{0}$ and initial velocity zero is then given by \cite{bib:bvmvn2007} as

\begin{equation}
\varphi(t) = 2\arcsin\left( c\cdot\mathrm{sn}\left( \mathrm{K}(c^2) - \omega_{0}t,\,c^2 \right) \right)
\end{equation}

with the two constants

\begin{equation}
\omega_0 = \sqrt{\frac{g}{l}}, \quad c = \sin\left(\frac{\varphi_{0}}{2}\right).
\end{equation}

We can obtain the angular velocity $\dot{\varphi}(t)$ by deriving $\varphi(t)$ and the angular acceleration $\ddot{\varphi}(t)$ from the equation of motion as

\begin{align}
\dot{\varphi}(t) &= -2c\omega_{0} \cdot \frac{\mathrm{cn}\left( \mathrm{K(c^2)} - \omega_{0}t,\,c^2 \right) \cdot \mathrm{dn}\left( \mathrm{K(c^2)} - \omega_{0}t,\,c^2 \right)}{\sqrt{1 - \left( c\cdot\mathrm{sn}\left( \mathrm{K(c^2)} - \omega_{0}t,\,c^2 \right) \right)^2}} \\
\notag \\
\ddot{\varphi}(t) &= -\omega_0^2\sin(\varphi(t))
\end{align}

In order to compare with the simulation results, we convert the results into cartesian coordinates of the mass

\begin{align}
x(t) &= \phantom{-}l\sin(\varphi), \quad \dot{x}(t) = l\dot{\varphi}\cos(\varphi), \quad \ddot{x}(t) = l\ddot{\varphi}\cos(\varphi) - l\dot{\varphi}^2\sin(\varphi) \\
y(t) &= -l\cos(\varphi), \quad \dot{y}(t) = l\dot{\varphi}\sin(\varphi), \quad \ddot{y}(t) = l\ddot{\varphi}\sin(\varphi) + l\dot{\varphi}^2\cos(\varphi)
\end{align}
...
The analytical solution for the free motion of the beam, without any external excitation, is found to be \textcolor{red}{[source]}
%
\begin{align}
...
The shape functions depend only on the length $l$ and the boundary conditions of the beam and are for this example given by
%
\begin{align}
...
% Characteristic equation: 1 + cos(k*l)*cosh(k*l) = 0 (https://wandinger.userweb.mwn.de/LA_Elastodynamik_2/kap_3_balken.pdf)
% Numerical solutions computed by Wolframalpha (https://www.wolframalpha.com/input?i=1+%2B+cos%28x%29*cosh%28x%29+%3D+0)
\begin{align}
\kappa_{1}l &\approx 1.87510406871196, \quad \kappa_{2}l \approx 4.69409113297417, \\
\kappa_{3}l &\approx 7.85475743823761, \quad \kappa_{4}l \approx 10.9955407348755, \notag \\
...
The free constants can be determined from the initial conditions of the beam, i.e. the initial displacement $w(x,\,0) = w_{0}(x)$ and velocity $\dot{w}(x,\,0) = v_{0}(x)$,
%
\begin{align}
...
For testing, a circular initial deformation with radius $r$ is used in combination with zero initial velocity,

\begin{align}
w_0(x) &= r - \sqrt{r^2 - x^2}, \\
v_0(x) &= 0.
\end{align}

The integrals above are solved numerically to a reasonable precision.