...
This spiral segment is a \textit{Clothoid}, also called \textit{Euler Spiral}, which is a curve whose curvature varies linearly over its length.
...
Integrating the curvature over the arc length leads to the tangent angle $\varphi$,
%
\begin{align}
...
Similarly, a line segment with zero curvature follows for the special case $a = 0$ and $b = 0$.
...
It does this by constructing two cubic splines~$x(t)$ and~$y(t)$ for each curve component.
...
More about splines in the following sections about width and thickness.

\section{Width and Thickness}

The width and thickness of the limb and its layers is described by monotonic cubic splines that interpolate user-supplied control points.
...
Jacobian of the elastic displacements:

\begin{equation}
\boldsymbol{J}(\boldsymbol{u}) = \frac{\partial\boldsymbol{u}_l}{\partial\boldsymbol{u}}
=
\begin{bmatrix}
...
Consider the elastic bar shown in figure~\ref{fig:viscoelastic-bar}.
...
We are going to follow the usual derivation for longitudinal vibrations of bars that can be found in many mechnaics textbooks, like \cite{bib:tm4}, except that we introduce a viscoelastic material into the mix.
...
Inserting this into the equation of motion (\ref{eq:viscoelastic-bar-motion}) and separating terms depending on $x$ from terms depending on $t$ leads to
...
This chapter is for unrealized ideas and improvements that have already been partly documented while exploring them, but didn't yet make it into VirtualBow for various reasons.
...
The method (not yet) used in VirtualBow first calculates a lower and upper bound on the displacement step as
...
were,~$n_\mathrm{min}$ and~$n_\mathrm{max}$ are parameters that determine the minimum resolution of the output as well as the maximum resolution before the procedure is aborted when the steps become too small.
...
For VirtualBow, a zero-order method called \textit{Armijo Backtracking} was chosen, since zero-order methods do not assume or require any sort of nice behaviour form the merit function.

The core of the scheme is trying to find a value for $\alpha$ that fulfills the \textit{Armijo condition}, also called the condition of sufficient decrease, which is written as
...
\subsection*{Constrainted Newton-Raphson}
...
The problem was that it never seemed to work when a line search was introduced that tries to force a monotonous decrease of the error in each iteration.
...
In any case, some more thought needs to be put into this before line seaching can make its way into VirtualBow.
...
The Newmark approximations (\ref{eq:newmark_v}), (\ref{eq:newmark_u}) are left unchanged, but the balance equation~(\ref{eq:newmark_balance}) is modified to
%
\begin{equation}
\boldsymbol{M}\boldsymbol{a}_{n+1-\alpha_m} + \boldsymbol{Q}_{n+1-\alpha_f} = \boldsymbol{P}_{n+1-\alpha_f},
\end{equation}

so the equation of motion is not evaluated at the interval end~$t_{n+1}$ but at two different points~$t_{n+1-\alpha_m}$ and~ $t_{n+1-\alpha_f}$ within the interval instead.
...
With this, the system of equations to solve at each time step, including its jacobian matrix, becomes
%
\begin{align}
