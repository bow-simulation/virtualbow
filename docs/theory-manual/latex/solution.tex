...
VirtualBow uses an absolute criterion on the residuial vector and works around the different dimensions by scaling the components accordingly.
We formulate the criterion as

\begin{equation}
\lVert \boldsymbol{S}^{-1}\boldsymbol{f} \rVert < 1
\end{equation}

using the Euclidean or L2 norm and a diagonal scaling matrix $\boldsymbol{S} = \rm{diag}(\varepsilon_{0},\ldots,\varepsilon_{n-1})$.
The norm being less than 1 implies that each component of the vector inside the norm must be less than 1, too.
Therefore the criterion guarantees that

\begin{equation}
|f_{i}| < \varepsilon_{i},\quad i = 0\,\ldots\,n-1.
\end{equation}

So the scaling factors $\varepsilon_{i}$ can be thought of as absolute tolerances for each component of the residual.
