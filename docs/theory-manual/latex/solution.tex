...
\subsubsection*{Load- and displacement control}
...
\Delta t_{i+1} &= \mathrm{clamp}\Big\{ f(\xi_{i})\,\Delta t_{i}, \, \Delta t_{\min}, \, \Delta t_{\max} \Big\}.
\end{align}
...
with $e(\boldsymbol{a})$ being an event function that takes in the current state of the system (in our case only the accelerationa are needed) and returns zero when the condition for the event is fulfilled.
The fist task is to detect the occurrence of this event while stepping through the numerical solution process.
We assume that an event has occurred betwen times~$t_{i}$ and~$t_{i+1}$ if there was a sign change of the event function,

\begin{equation}
\sgn\left\{ e(\boldsymbol{a}_{i}) \right\} \neq \sgn\left\{ e(\boldsymbol{a}_{i+1}) \right\}.
\end{equation}

Technically speaking, any odd number of events could have occurred during that timestep.
...
None of that is pracically relevant for our bow simulation though since the timesteps are usually small enough to make multiple events very unlikely.
...
When an event that should stop the simulation (e.g. arrow separation) is detected between~$t_{i}$ and~$t_{i+1}$, we know that the last time step was actually too large.
...
But by looking back at figure \ref{fig:adaptive-timestep} we can see that this approach would lead to large errors when using the Newmark method, since the timesteps are far too large.
