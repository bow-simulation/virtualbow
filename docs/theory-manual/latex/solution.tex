...
\textcolor{red}{Stability od implicit and explicit methods.}
...
Selecting the timestep $\Delta t$ is a tradeoff between accuracy and computational cost.
Smaller timesteps results in a more accurate and detailed solution, while larger timesteps reduce the number of required system evaluations and therefore the computational cost.

Ideally we would chose a timestep that is as large as possible while still maintaining a prescribed degree of accuracy.
However, this ideal timestep will generally vary over time.
Sections of the solution that are relatively smooth and uneventful allow for larger timesteps while sections with frequent changes require a smaller timestep in order to accurately resolve all the details.
...
We can't possibly asses them all here, but \cite{bib:rossi2014} gives a good overview of various methods for structural dynamics and also compares three of them in more detail by solving various example problems.
Of the three methods investigated, the one by Bergan and Mollestad \cite{bib:bergan1985} seems to be the most well-established, is conceptually simple and overall doesn't perform worse than newer methods in the numerical comparison.
...
The current characteristic frequency is based on Rayleigh's quotient and provides an approximation for the frequency that currently dominates the system's dynamic response based on the last displacement increment.
...
In figure \ref{fig:adaptive-timestep}, the current characteristic frequency and the resulting optimal timestep are shown for each step of the dynamic simulation of a simple example bow.
...
\caption{Example for the estimated frequency~$\omega$ and the associated timestep~$\Delta t^*$ for~$N = 50$ during an example simulation. Marked in red is the departure of the arrow.}
\label{fig:adaptive-timestep}
\end{figure}

It can be seen that the characteristic frequency varies greatly over the duration of the simulation, which confirms that adaptive time stepping can be useful for dynamic bow simulations.
...
After this initial disturbance settles down, the motion of the bow turns into a steady and continuous acceleration of the arrow, therefore a relatively smooth section with a low characteristic frequency and larger optimal time steps follows.
...
In the case of nonlinear dynamic analysis the factorizations have to be done at each timestep anyway, which makes this less important. \textcolor{red}{TODO: Document actual choice of this parameter, whether used or not.}
\item A limitation of the increase in step size to a factor of $\xi_{m}$, which is typicallay chosen as $2$.
This prevents the timestep from prematurely increasing too much and then having to decrease again later.
...
This method as described here is used in VirtualBow since version 0.10.
