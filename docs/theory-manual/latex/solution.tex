...
Formulating a convergence criterion that works for all edge cases and whose numerical tolerances scale well with different systems is surprisingly tricky.

Convergence criteria are commonly formulated in terms of either the displacement step~$\Delta\boldsymbol{u}$, the residual forces~$\boldsymbol{R}$ or their product~$\Delta\boldsymbol{u}^\intercal\boldsymbol{R}$, which has the dimension of an energy.
Convergence criteria can furthermore be based either on the absolute error of the respective quantity or on a relative error with respect to the start of the iteration.
...
\hline
$\boldsymbol{R}$ & $\lVert \boldsymbol{R}_{i} \rVert < \varepsilon_\mathrm{abs}$ & $\lVert \boldsymbol{R}_{i} \rVert \,/\, \lVert \boldsymbol{R}_{0} \rVert < \varepsilon_\mathrm{rel}$ \\
$\Delta\boldsymbol{u}$ & $\lVert \Delta\boldsymbol{u}_{i} \rVert < \varepsilon_\mathrm{abs}$ & $\lVert \Delta\boldsymbol{u}_{i} \rVert \,/\, \lVert \Delta\boldsymbol{u}_{0} \rVert < \varepsilon_\mathrm{rel}$ \\
$\Delta\boldsymbol{u}^\intercal\boldsymbol{R}$ & $\lVert \Delta\boldsymbol{u}_{i}^\intercal\boldsymbol{R}_{i} \rVert < \varepsilon_\mathrm{abs}$ & $\lVert \Delta\boldsymbol{u}_{i}^\intercal\boldsymbol{R}_{i} \rVert \,/\, \lVert \Delta\boldsymbol{u}_{0}^\intercal\boldsymbol{R}_{0} \rVert < \varepsilon_\mathrm{rel}$ \\
\hline
\end{tabular}
...
Each choice has advantages and disadvantages, which will make it necessary to combine some of those criteria.
...
That's why they have to be combined with an absolute criterion.
...
VirtualBow currently uses the absolute and relative error of the residual forces as the convergence criterion.
The iteration is stopped when either of those falls below a certain tolerance.
The infinity or maximum norm $\lVert \boldsymbol{R} \rVert_{\infty}$ is chosen as the vector norm, which is the maximum absolute value of the vector.
The scaling issue between translational and rotational degrees of freedom was solved by completely ignoring it.

\textcolor{red}{Mention normalizing/scaling the coordinates as a possible solution}

\textcolor{red}{Normalization with respect to coordinates:} When using the euclidean norm of the residual vector $\lVert\boldsymbol{f}\rVert = \sqrt{\sum_{i=0}^{n-1}f_{i}^2}$ as the convergence criterion with a constant (general purpose) tolerance $\varepsilon$, the criterion is stricter for residuals of higher dimensions.
Imagine a residual vector with a constant error of $\varepsilon$ in each component:

\begin{equation}
\lVert\boldsymbol{f}\rVert = \sqrt{\sum_{i=0}^{n-1}\varepsilon^2} = \sqrt{n\cdot\varepsilon^2} = \sqrt{n}\cdot\varepsilon
\end{equation}

So we have to compare the residual norm with $\sqrt{n}\cdot\varepsilon$ in order to circumvent scaling issues.
...
\begin{equation}
g(\alpha) = \lVert \boldsymbol{f}(\boldsymbol{x}_{i} + \alpha\Delta\boldsymbol{x}_{i}) \rVert^2.
\end{equation}
...
Usually $c$ is chosen between $0.1$ and $0.0001$ \textcolor{red}{Source?}.
Using \textcolor{red}{Link} and \textcolor{red}{Link}, the condition expands to

\begin{equation}
\lVert \boldsymbol{f}(\boldsymbol{x}_{i} + \alpha\Delta\boldsymbol{x}_{i}) \rVert^2 \le \lVert \boldsymbol{f}(\boldsymbol{x}_{i}) \rVert^2 + 2c\alpha\,\boldsymbol{f}(\boldsymbol{x}_{i})^\intercal\boldsymbol{J}(\boldsymbol{x}_{i})\,\Delta\boldsymbol{x}_{i}.
\end{equation}
...
\begin{equation}
\lVert\overline{\boldsymbol{f}}\rVert
=
\bigg\lVert
\begin{bmatrix}
\boldsymbol{f} \\
...
=
\sqrt{\boldsymbol{f}^\intercal\boldsymbol{f} + c^2}
< \sqrt{n+1}\cdot\varepsilon
\end{equation}

Slope term of the Armijo condition:

\begin{equation}
g'(0) = \boldsymbol{f}^\intercal\left(\frac{\partial\boldsymbol{f}}{\partial\boldsymbol{x}}\Delta\boldsymbol{x} + \frac{\partial\boldsymbol{f}}{\partial\lambda}\Delta\lambda\right) + c\left(\frac{\partial c}{\partial\boldsymbol{x}}\Delta\boldsymbol{x} + \frac{\partial c}{\partial\lambda}\Delta\lambda\right)
\end{equation}
