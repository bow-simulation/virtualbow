...
Formulating a convergence criterion that works for all edge cases and that whose numerical tolerances scale well with different systems is surprisingly tricky.
...
\hline
\end{tabular}
\caption{Different absolute and relative convergence criteria for the Newton-Raphson method. The vector norm $\rVert.\rVert$ can be chosen freely.}
...
This means that we can find a generic value for the tolerance $\varepsilon_\mathrm{rel}$ that will work most of the time.
The absolute tolerances $\varepsilon_\mathrm{abs}$ on the other hand represent a different accuracy depending on what a typical force, displacement or energy in the system is.
Unfortunately though, relative criteria don't work for the very first iteration or when the system is already in static equilibrium.
That's why they have to be combined with an absolute criterion.

Since the thing we want to compute are the equilibrium displacements, it would make sense to use a criterion on $\Delta u$ and stop when the displacements do not change meaningfully anymore.
