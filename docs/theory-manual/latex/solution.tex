...
Selecting the timestep $\Delta t$ is a tradeoff between accuracy and computational effort.
Ideally we would chose a timestep that is as large as possible but also small enough such that the computed solution is still reasonably accurate.
This minimum required timestep will generally vary over time.
...
\begin{equation}
\omega = \left\vert \frac{\Delta \boldsymbol{u}^\intercal \boldsymbol{K} \Delta \boldsymbol{u}}{\Delta \boldsymbol{u}^\intercal   \boldsymbol{M} \Delta \boldsymbol{u}} \right\vert, \quad T = \frac{2\pi}{\omega}
\end{equation}
