...
The goal of a modal analysis is to obtain the natural frequencies and mode shapes of a system.
...
Also the results are only meaningful for small displacements relative to that setpoint.
...
Since this is a linear, homogenous differential equation, we can assume its solutions to take the form $\boldsymbol{u}(t) = \hat{\boldsymbol{u}}\,e^{\lambda\,t}$ with $\hat{\boldsymbol{u}} \in \mathbb{R}^n$ and $\lambda \in \mathbb{C}$. Substituting this into the equation gives us the condition
