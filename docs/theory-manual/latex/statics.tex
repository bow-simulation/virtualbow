...
Different numerical methods are needed for this, depending on whether a static or dynamic analysis is to be performed.

The first part of the bow simulation, which encompasses bracing the bow and drawing it from brace height to full draw, is static in nature.
That's because the process is fairly slow and therefore the influence of the moving masses of the bow can be neglected.
A system such as ours, with an equation of motion of the form
...
In words, all components of the internal forces~$\boldsymbol{Q}$ have to be in balance with the respective external loads~$\boldsymbol{P}$.
We introduce the residual forces~$\boldsymbol{R}(\boldsymbol{u})$ as a shorthand for the difference between the two.
...
In the most basic case, the external loads~$\boldsymbol{P}$ are given and the task is to calculate the corresponding equilibrium displacements.
This case is called \textit{load-controlled}.
Equation~\eqref{eq:statics:equilibrium} becomes a nonlinear system of equations in~$\boldsymbol{u}$ exclusively.
Except for very simple systems, this can only be solved numerically.
A commonly used method for this kind of problem is the \textit{Newton-Raphson method}. The idea behind it is to linearize the equilibrium equation~\eqref{eq:statics:equilibrium} around a known displacement vector~$\boldsymbol{u}_i$ and solve the resulting linear equation in order to obtain an improved approximation~$\boldsymbol{u}_{i+1}$.
Starting with an initial value~$\boldsymbol{u}_0$, the procedure is repeated until the solution is accurate enough, as determined by some convergence criterion.
Linearization of~\eqref{eq:statics:equilibrium} yields
...
Relevant keywords in this context are \textit{simplified Newton methods} and \textit{quasi-Newton methods} such as the \textit{BFGS method}.
The first extension we are going to look at, which is essential for our bow simulation, is turning our load-controlled  method into a \textit{displacement controlled} one.

\newpage
\section{Load- and Displacement Control}
\label{sec:constrained-newton-raphson}

When simulating the drawing of the bow, the set of known and unknown variables is not quite the same as assumed in the previous section.
...
We do know, however, the position of the string node, since that corresponds to the current draw length at which the bow is to be evaluated.
A solution to this is \textit{displacement control}, where we prescribe a value for one of the displacement components and instead treat the magnitude of the external forces as a variable that is determined during solution.
Displacement control also has the benefit that it still works when there is a decrease in load in the force-draw curve, which can theoretically happen with some extreme recurve designs.

Before the bow is drawn, there is actually another static simulation step: bracing.
...
The bracing simulation is described in more detail later; it too makes use of displacement control.
...
Here the external forces are scaled by the unknown load factor~$\lambda$. To compensate for this additional unknown, a scalar constraint~$c(\boldsymbol{u},\,\lambda) = 0$ on the displacements and the load factor is introduced.
Depending on the choice of constraint, the method can be turned into \textit{load control}, \textit{displacement control} or other more sophisticated schemes like the \textit{arc-length method}.
We will carry out the derivation in a generic way and specify the constraints only as the last step.
Let us first restate equations~\eqref{eq:equilibrium_dc} and~\eqref{eq:constraint_dc} in more general terms as
%
\begin{align}
...
As before, the Newton-Raphson method is used: The equations are linearized around a given approximation~$\{\boldsymbol{x}_i,\,\lambda_i\}$ and the resulting linear system is solved to obtain an improved approximation~$\{\boldsymbol{x}_{i+1},\,\lambda_{i+1}\}$.
This iterative process is repeated until~$\boldsymbol{x}$ and~$\lambda$ satisfy both the vector equation and the scalar constraint reasonably well as determined by some convergence criterion.
...
In order to solve this for the solution increments~$\Delta\boldsymbol{x}_{i}$ and~$\Delta\lambda_{i}$, we could rearrange the equations~\eqref{eq:equilibrium_dc_lin} and~\eqref{eq:constraints_dc_lin} into a single linear system
%
\begin{align}
...
From equation~\eqref{eq:equilibrium_dc_lin}, the unknowns~$\Delta\boldsymbol{x}_{i}$ can be expressed as
%
\begin{align}
...
Substituting~\eqref{eq:auxiliary_disp_dc} into the second equation~\eqref{eq:constraints_dc_lin} gives us the parameter increment as
...
This is then used to compute~$\Delta\boldsymbol{x}_{i}$ according to~\eqref{eq:auxiliary_disp_dc}.
From equation~\eqref{eq:res_load_factor} we can see that the parameter increment~$\Delta \lambda_{i}$ is invalid if the denominator of the expression vanishes, i.e.~$\frac{\partial c}{\partial \lambda} + \frac{\partial c}{\partial \boldsymbol{x}}\,\boldsymbol{\beta} = 0$.
This needs to be checked in each iteration and usually points to an ill-posed problem with regard to the imposed constraint function.
...
%
\begin{align}
c(\boldsymbol{u},\,\lambda) = u^{(k)} - \overline{u}
\end{align}
%
...
%
\begin{align}
c(\boldsymbol{u},\,\lambda) = \lambda - \overline{\lambda},
\end{align}
%
...
$\Delta\boldsymbol{x}^\intercal\boldsymbol{f}$ &~$\lVert \Delta\boldsymbol{x}_{i}^\intercal\boldsymbol{f}_{i} \rVert < \varepsilon_\mathrm{abs}$ &~$\lVert \Delta\boldsymbol{x}_{i}^\intercal\boldsymbol{f}_{i} \rVert \,/\, \lVert \Delta\boldsymbol{x}_{0}^\intercal\boldsymbol{f}_{0} \rVert < \varepsilon_\mathrm{rel}$ \\
\hline
\end{tabular}
\caption{Different absolute and relative convergence criteria for the Newton-Raphson method. The vector norm~$\lVert.\rVert$ can also be chosen freely.}
...
Relative criteria are more appealing because they are dimensionless and independent of the overall scale of the system.
...
Unfortunately, though, relative criteria are not defined for the very first iteration and also don't work when the system is already in (or close to) static equilibrium at the start of the iterations.
That's why they have to be combined with an absolute criterion anyway.

Since what we want to compute are the equilibrium displacements, it would make sense to use a criterion on~$\Delta\boldsymbol{x}$ and stop when the displacements do not change meaningfully anymore.
It is, however, possible that the iterative displacement change is very small while the out-of-balance loads~$\boldsymbol{f}$ are still large, which is why Crisfield~\cite{bib:chrisfield1991} advises against using a displacement-based criterion on its own without supplementing it with a force-based one.

Criteria based on the residual forces or displacement increments also suffer from an additional scaling problem.
Both of these vectors contain translational as well as rotational entries, so the application of a simple vector norm~$\lVert.\rVert$ does not make much sense since entries with different units (length vs. angle or force vs. torque) are added together.
This scaling issue between translational and rotational degrees of freedom is absent in the energy-based criterion:
In the expression~$\Delta\boldsymbol{x}^\intercal\boldsymbol{f}$, lengths are multiplied by forces and angles are multiplied by torques, resulting in a vector where each entry has the dimension of an energy.
...
Or, as Crisfield~\cite{bib:chrisfield1991} puts it, the condition merely implies that a stationary energy position has been reached along the current search direction~$\Delta\boldsymbol{x}$, which frequently occurs when the solution is still a very long way from equilibrium.
He therefore warns that such criteria should be used with great caution.

VirtualBow currently uses only a single, absolute criterion based on the displacement increments~$\Delta\boldsymbol{x}$.
The reason for that is that we can actually derive reasonable absolute tolerances from the dimensions of the bow model.
We first define a generic numerical tolerance~$\varepsilon$, which is called ``static iteration tolerance'' or ``dynamic iteration tolerance'' in the UI.
From this we derive absolute tolerances for lengths, angles, accelerations and the load factor (which will be needed later) based on a reference length~$l_{\rm{ref}}$, reference angle~$\varphi_{\rm{ref}}$ and reference acceleration~$a_{\rm{ref}}$ that represent the overall scale of the model,
...
The reference acceleration for dynamic iterations is the initial acceleration of the arrow, i.e. the draw force divided by the arrow mass.
...
This solves the problem of mixing dimensions because we can use different tolerances for lengths and angles and the end result will be dimensionless.
If the method is constrained, the analogous convergence condition

\begin{equation}
|\Delta\lambda / \varepsilon_{\rm{abs},\lambda}| < 1
\end{equation}

for the load factor has to be fulfilled too, in addition to the condition on the displacements.

\section{Derivative of the Equilibrium Displacements}

The derivative~$\partial \boldsymbol{x}/\partial \lambda$ of the equilibrium displacements with respect to the load factor is used by VirtualBow to obtain the stiffness of the force-draw curve, i.e. the increase in force (load factor) per increase in equilibrium displacement.
...
Solving for~$\partial \boldsymbol{x}/\partial \lambda$ we find that the derivative we are looking for is actually equal to the auxiliary vector~$\boldsymbol{\beta}$ as defined in~\eqref{eq:auxiliary_disp_dc},
...
For VirtualBow, this gives the exact stiffness of the force-draw curve without having to compute a numerical derivative and irrespective of how fine or coarse the resolution of the curve is.

\newpage
\section{Finding the Braced Equilibrium of a Bow}

The first part of the static simulation also tends to be the trickiest one: Finding the first equilibrium state of the bow at the given brace height.
Older versions of VirtualBow in particular would frequently struggle with this step and give users cryptic error messages from failed solution attempts.
The bracing method described below is the result of a lot of trial and error and seems to be the most robust so far, even though it is still not perfect.

One difficulty of bracing is that the braced state of the bow tends to be ``far away'' from the initial, unbraced state.
Newton-Raphson iterations, however, are much more efficient and robust when they start from a good initial guess, i.e. an initial state that is not very far from the desired equilibrium.
...
The current bracing method tries to solve both of these problems by applying an initially overlong string to the bow and iteratively shortening it until the desired brace height is reached.
How much the string is shortened in each step is controlled/limited in a way that increases the chances for convergence.
...
This initial length is chosen such that there is no tension yet in the string.
...
At the point where the string intersects the~$y$ axis, it initially has a positive slope of~$\Delta y / \Delta x$ as measured relative to the~$x$-direction.
If this initial slope were negative, the bow model would have to be rejected as invalid because the specified brace height would be too low for the given limb geometry.
The actual bracing process now consists of successively shortening the string and iterating for equilibrium each time, until the slope reaches zero, while also keeping the string center pinned at the desired brace height via displacement control.
...
\newpage
As the string becomes shorter, the slope decreases until it eventually crosses into the negative.
...
\caption{Slope versus string length for a real example}
...
Instead, the method starts at~$p = 1$ and treats the problem as one of finding an equilibrium path with the path parameter~$p$.
This parameter is controlled by taking into account maximum and minimum bounds on the step size and the performance of the static iterations.
A dedicated root-finding algorithm is used only after the first negative slope is encountered in order to home in on a precise final result.
Currently this is the \textit{regula falsi method}, which is simple and reasonably effective when derivatives are not available and the interval in which the root lies is known.