...
\label{chap:element-equations}

\section{Mass Element}

The point mass is the simplest element type used in the bow model.
Figure~\ref{fig:elements:point-mass} shows its definition.
It only consists of a single node, denoted by an index of zero, whose position is described by the Cartesian coordinates~$x_0(t)$ and~$y_0(t)$.
...
So this element only contributes to the mass matrix of the overall system.
Note also that the element's mass matrix is diagonal, because there is no coupling between the two directions of motion.
...
It is easy to imagine that this elastic deformation would decrease even further if the length of the tracked segment were made shorter.
The large overall deflection of the limb seems to be the result of the compounding effect of all the small, local deflections.
The corotational formulation makes use of this basic principle.
...
The big advantage of this approach is that the local, elastic deformations can be treated as linear, which is both conceptually simpler and computationally more efficient.
...
This is not the only way that a corotational reference frame can be defined, but it seems to be the most common one~\cite{bib:le2011, bib:yaw2009} and compares favorably with the alternatives~\cite{bib:berzeri2001}.
...
Due to the possible initial curvature, its centerline forms an initial angle with the $\overline{x}$-axis of~$\beta_0$ at the first node and of~$\beta_1$ at the second node.
...
\begin{equation}
\overline{\boldsymbol{P}} = \overline{\boldsymbol{Q}}(\boldsymbol{u}_{e},\,\dot{\boldsymbol{u}}_{e}) = \overline{\boldsymbol{K}}\cdot\overline{\boldsymbol{u}} + \overline{\boldsymbol{D}}\cdot\dot{\overline{\boldsymbol{u}}}, \label{eq:corotational-linear-forces}
\end{equation}
...
Instead we will continue by developing the kinematic relations required for a corotational formulation.
...
These global positions and rotations are the coordinates in which we actually want to formulate the equation of motion of the element:
...
The rigid-body rotation~$\alpha$ can be calculated via~$\arctan2$ as the angle of the~$\overline{x}$-axis passing through the two nodes.
...
The elastic elongation~$\Delta d$ is the difference between the current distance~$d$ of the nodes and the initial distance~$d_0$.
...
One critical subtlety must be addressed here: the resulting angular displacements need to be \textit{normalized}, i.e. wrapped to an interval of~$]-\pi,\,\pi]$.
...
By combining~\eqref{eq:corotational-force-transform} with the linear forces~\eqref{eq:corotational-linear-forces} and rewriting~$\boldsymbol{u}_{e}$ and~$\dot{\boldsymbol{u}}_{e}$ in terms of global coordinates, we obtain
...
This stands in contrast to formulations in which the internal forces have to be evaluated numerically, often through \textit{Gaussian quadrature} or related integration schemes.
...
In particular, they cannot be captured by a constant mass matrix, as has been the premise up to this point.
So instead of using a consistent mass matrix, we are going to construct a \textit{lumped mass matrix}, which is a common approximation where an element's mass and inertia are distributed, i.e. \textit{lumped}, to its nodes.
...
Unlike more general matrices, a diagonal matrix can be trivially inverted just by independently inverting its diagonal entries.
...
Wikipedia~\cite{bib:castigliano} states it as follows:

\textit{If the strain energy of a linearly elastic structure can be expressed as a function of generalized force~$P_i$ then the partial derivative of the strain energy with respect to generalized force gives the generalized displacement~$u_i$ in the direction of~$P_i$.} (Notation adapted)
...
With this we have identified the inverse~$K^{-1}$ of the bar's stiffness in this loading case.
...
If we assume~$EA$ to be constant for a quick sanity check, we get a stiffness of~$K = EA/l$, which is the expected result for the stiffness of a uniform bar.
...
The actual displacement is then obtained by setting the auxiliary force~$F^*$ to zero, resulting in the displacement distribution within the bar,
...
Since Castigliano's method works not only for bars, but for any linear-elastic structure, it will be an important tool for deriving the stiffness matrices required for the corotational beam element.

\subsection{Stiffness Matrix of a Beam Segment}

We can now use Castigliano's theorem to calculate the stiffness matrix and the deformation of a general, linear-elastic beam segment.
The segment can be initially curved and have varying cross-section properties along its length.
...
It has two nodes and six nodal degrees of freedom, for which we are going to derive the complete~$6 \times 6$ stiffness matrix.
...
The stiffness properties of the cross-sections are defined by their stiffness matrix~$\boldsymbol{C}(s) \in \mathbb{R}^{3 \times 3}$, which is assumed to be known and establishes a linear constitutive relation of the form
...
between the cross-section forces~$\boldsymbol{f}$ and the generalized strains~$\boldsymbol{e}$ at each length~$s$.
...
You may wonder why we introduced shear deformation here, even though the bow model was said to be based on Euler-Bernoulli theory earlier.
...
The potential elastic energy (strain energy) of the beam segment, which we are going to need later, can be evaluated by integrating the product~$\boldsymbol{f}^\intercal\boldsymbol{e}$ of section forces and strains over the beam's length and using the stiffness relation~\eqref{eq:section-stiffness-matrix} to eliminate the unknown strains:
...
We assign two nodes to this segment, one at the starting point~$s = s_0$ and one on the endpoint~$s = s_1$ of the curve.
...
The goal is now to find the stiffness matrix of the segment, which describes the linear relationship between the combined nodal forces and displacements.
...
We can get a physical interpretation of the submatrices~$\boldsymbol{K}_{ij}$ by fixing one node at a time as shown in Figure~\ref{fig:beam-segment-linear-cases}.
...
In \textbf{Case I}, where the first node is fixed ($\Delta\boldsymbol{p}_{0} = \boldsymbol{0}$), the resulting forces are
%
\begin{align}
...
In \textbf{Case II}, where the second node is fixed ($\Delta\boldsymbol{p}_{1} = \boldsymbol{0}$), the resulting forces are
%
\begin{align}
...
We can use this relationship to calculate the constraint forces at the clamped nodes in cases I and II in order to obtain the off-diagonal stiffnesses~$\boldsymbol{K}_{1,0}$ and~$\boldsymbol{K}_{0,1}$ as follows,
...
In the first case, the beam is clamped at node~0 and a force is applied at node~1.
Due to static equilibrium, the distribution of cross-section forces in the beam for an external force~$\boldsymbol{F}_\eta$ that is applied at the position~$s_\eta$ with~$\eta \in [0,\,1]$ is found to be
...
\end{bmatrix}
}_{\boldsymbol{F}_\eta} \label{eq:section-static-equilibrium-1}
\end{equation}
...
The strain energy for this load case according to~\eqref{eq:section-potential-energy} is therefore
...
F_{x,\eta} \\ F_{y,\eta} \\ M_{z,\eta}
\end{bmatrix}
}_{\boldsymbol{F}_\eta}, \label{eq:section-static-equilibrium-2}
\end{equation}
...
from which the stiffness matrix~$\boldsymbol{K}^{-1}_{0,0}$ for the left node follows for~$\eta = 0$.
...
Alternatively, since the stiffness matrix must be symmetric, the relation~$\boldsymbol{K}_{0,1} = \boldsymbol{K}_{1,0}^\intercal$ can be used for calculating one of the off-diagonal blocks in terms of the other.
Verifying that this is true is left as an exercise for the reader \texttt{;)}
The full displacements of the beam segment under forces at both nodes are obtained by adding together the displacements from both load cases due to linear superposition,
...
with the displacement evaluation matrix~$\boldsymbol{E}_{u,\eta}$ at the point $\eta$.
...
Note that up to this point all results have been exact, in the sense that no further simplifications within the linear-elastic beam theory have been made.
The integrals over the segment's length account for arbitrary variations of shape and cross-section properties.
In the actual implementation, though, the integration itself is done numerically, because accounting for all kinds of shapes and cross-sections analytically would be too complicated.
...
To obtain the required integrals from above, these partial results are summed up as needed.
...
Therefore the integration is not performance-critical and can be done fairly accurately with small numerical tolerances.
...
%For the section forces we can use equation~\eqref{eq:section-static-equilibrium-1} and the total stiffness equation~\eqref{eq:segment-stiffness-matrix} to get
...
%Knowing the cross-section forces, the strains can be computed from~\eqref{eq:section-stiffness-matrix} by inverting the section's stiffness matrix,
...
We now have a general~$6 \times 6$ stiffness matrix of an arbitrary beam segment, but according to Figure~\ref{fig:beam-element-global-frame} we need the stiffness matrix relative to a coordinate system that is rotated by the initial angle of~$\alpha(t=0)$ and reduced to the three local coordinates~$\overline{\boldsymbol{u}} = \begin{bmatrix}\Delta d, & \Delta \beta_0, & \Delta \beta_1\end{bmatrix}^\intercal$.
...
with the following derivative terms:
...
These transformations enable the evaluation of the beam's deflection curve between the nodes, leading to finer-grained simulation output irrespective of the number of elements being used.
...
The only assumption about the beam's cross-sections was that they exhibit a linear relationship between forces and strains, as formalized by equation~\eqref{eq:section-stiffness-matrix}, where this relationship is described by the stiffness matrix~$\boldsymbol{C} \in \mathbb{R}^{3 \times 3}$ of the section.
...
Each layer has its own density~$\rho_i$, Young's modulus~$E_i$ and shear modulus~$G_i$.
...
The distribution of normal strain over the section is therefore given by the continuous function

\begin{equation}
\overline{\varepsilon}(y) = \varepsilon - \kappa\,y,
\end{equation}

where~$\varepsilon$ is the strain at the beam's centerline in~$x$-direction and~$\kappa$ its curvature around the~$z$-axis.
...
In order to determine the elastic constants~$C_{ij}$ of the section's stiffness matrix, we calculate the total normal force~$N$ and bending moment~$M$ acting on the cross-section by integrating the stresses~\eqref{eq:beam-stress-distribution} over the section's area.
...
Note that the coupling between tension and bending would vanish if the beam's reference line passed through the centroid of each section.
This is often assumed for simplicity, but in the case of VirtualBow the beam's reference line is the user-defined profile curve, which does not generally have this property.
...
But as already mentioned, VirtualBow doesn't make use of the shear component right now; it merely acts as a placeholder.
...
The plan is to make this approximation obsolete in the future by switching to a fully numerical cross-section analysis instead, which accurately computes all stiffnesses and stress distributions of the section, including those for shear and torsion, and doesn't make any kinematic simplifications as in the Euler-Bernoulli or Timoshenko theories.
...
Combining them into the section's stiffness matrix results in
...
In the case of the rectangular layers shown above, the area~$A_i$ and the second moment of area~$I_i$ evaluate to

\begin{align}
A_i &= w_i\,h_i, \quad I_i = A_i\left(\frac{h_i^2}{12} + y_i^2\right).
\end{align}

To complete the element's mass matrix, we're also going to need the linear density (mass per unit length) of the section.
This can be calculated in a straightforward way by summing up the densities of the individual layers as
...
When trying to come up with the diagonal entries of our lumped mass matrix~\eqref{eq:corotational-mass-matrix}, we are basically asking how to pick concentrated masses and moments of inertia~$m_0$,~$J_0$ and~$m_1$,~$J_1$ at the nodes of the element such that it behaves like the actual beam segment, whose mass is continuously distributed along the curved and moving beam axis.
...
We distribute it between the nodes by assigning each one the mass of its half of the segment,

\begin{equation}
m_0 = \int_{0}^{\frac{l}{2}} \mu(s)\,ds, \quad m_1 = \int_{\frac{l}{2}}^{l} \mu(s)\,ds.
\end{equation}

This conserves the total mass of the element, which is an important requirement.
It does not, however, conserve the rigid-body inertia of the element.
That's because concentrating the mass at the two nodes has turned the element into something like a barbell, with all the mass pushed to the ends, thereby increasing the inertia.

Determining the rotary inertias~$J_0$ and~$J_1$ is much less straightforward.
One reason is that there is no comparable ``total inertia'' to divide up.
The other reason is that the total rigid-body inertia of the element is already too large, as explained above, even before adding~$J_0$ and~$J_1$.
So in a certain sense, the most accurate values for these would be zero or even negative, which is obviously not permissible.
This is the fundamental dilemma when lumping the masses of a beam, which is also discussed in~\cite{bib:colorado31}.
According to that reference, a common choice for straight, uniform Euler-Bernoulli beams is

\begin{equation}
J_{0} = J_{1} = \alpha \cdot \rho A l^3
\end{equation}

with a fudge factor~$\alpha$ that should be small due to the dilemma described above and is typically chosen between~$0$ and~$\nicefrac{1}{50}$.
The expression~$\rho A l^3$ is proportional to the beam's rigid rotary inertia, but the exact point of reference is baked into~$\alpha$.

\newpage
We attempt to generalize this for our case of a non-uniform and curved beam by defining the nodal moments of inertia as

\begin{align}
J_0 &= \alpha \int_{0}^{l} \mu(s) \lVert \boldsymbol{r}(s) - \boldsymbol{r}(0) \rVert^2\,ds, \\
J_1 &= \alpha \int_{0}^{l} \mu(s) \lVert \boldsymbol{r}(s) - \boldsymbol{r}(l) \rVert^2\,ds,
\end{align}

where~$\alpha$ is a different fudge factor and the integrals are the rigid-body inertia of the segment with respect to each of the nodes.

\subsection{Damping Matrix}
\label{sec:rayleigh-damping}

The last missing piece for the beam element is the damping matrix~$\overline{\boldsymbol{D}}$ of the beam segment, analogous to its stiffness matrix, as required for the calculation of the local forces~\eqref{eq:corotational-linear-forces}.
Since the stiffness matrix has been derived from the segment's material properties and geometry, it would seem logical to do the same for the damping matrix as well.
But characterizing the dissipative properties of materials is much trickier than characterizing their elastic properties.
...
Therefore the damping matrix can only ever be a crude approximation, such that at least some energy dissipation is included in the bow model.
...
The damping model that is utilized for this is called \textit{Rayleigh damping} or \textit{proportional damping}, which is a simple approach that is very popular when little is known about the underlying physical details of the damping.
The idea is to write the damping matrix of a linear(ized) system as a weighted sum of its stiffness and mass matrices,

\begin{equation}
\boldsymbol{D} = \alpha\boldsymbol{K} + \beta\boldsymbol{M}\label{eq:rayleigh-damping-matrix}
\end{equation}

with the factor~$\alpha$ for \textit{stiffness-proportional} damping and the factor~$\beta$ for \textit{mass-proportional} damping.
The stiffness-proportional part dampens the system as if the material behaved in a viscous way while the mass-proportional part dampens the system as if the system was moving in a viscous medium.
It can be shown~\cite{bib:rayleigh-damping} that the natural frequencies~$\omega_i$ and the corresponding damping ratios~$\zeta_i$ in such a system are related by 

\begin{equation}
\zeta_{i} = \frac{1}{2}\left(\alpha\,\omega_{i} + \frac{\beta}{\omega_{i}}\right).
\end{equation}

This relationship can be used to identify~$\alpha$ and~$\beta$ from prescribed values of the damping ratio at specific modes.
In our case we use stiffness-proportional damping only, so~$\beta$ is set to zero.
If we assign a damping ratio of~$\zeta_{0}$ to the first mode of the system, for which we know the undamped natural frequency $\omega_0$, we can determine the required Rayleigh parameter~$\alpha$ as
...
With this undamped limb model, before the string is attached, a modal analysis is performed to determine its first undamped natural frequency.
Using this frequency and the prescribed damping ratio, the required damping coefficient~$\alpha$ is determined from equation~\eqref{eq:rayleigh-alpha-identification}.
The damping matrices of each beam element are then initialized as~$\overline{\boldsymbol{D}} = \alpha\overline{\boldsymbol{K}}$.
When the system is linearized, as is the case here, the factor~$\alpha$ carries over from the corotational elements to the system level as in~\eqref{eq:rayleigh-damping-matrix}.
...
\caption{Elongation of the string under load}
...
This model is called the \textit{Kelvin-Voigt model} and combines linear elastic and linear viscous behavior.
...
The product~$EA$ is also called the \textit{longitudinal stiffness}.
To use this as the string model in our bow simulation, we have to pick a set of nodes and formulate the internal forces in terms of the nodal coordinates.
It seems reasonable to place the string between two nodes, one start point and one endpoint, but there is an additional complication: the string model must also handle recurve bows, so we have to consider its possible contact with the bow limb.
This means that the current shape of the string, as well as its internal forces, don't only depend on its own start point and endpoint, but also on the nodes of the limb.
...
One shortcoming of this approach is already apparent: the contact with the string is determined only by the positions of the limb nodes, not by the actual deformed geometry of the limb's beam elements.
This is a possible future improvement that would allow us to accurately resolve the string contact with a much smaller number of limb nodes/elements.

We define the coordinate vector~$\boldsymbol{u}$ of the element, describing the string's current configuration, as
...
The contact point candidates at the belly surface of the limb are given by
...
The task of finding the contact points is then equivalent to finding the convex hull of the points~$\{x_{0},\,y_{0},\,\bar{x}_{1},\,\bar{y}_{1},\,\ldots,\,\bar{x}_{n-1},\,\bar{y}_{n-1}\}$.
This is currently implemented by a method similar to the \textit{Graham scan}~\cite{bib:grahamscan}, but adapted for 1D lines.
The scan is performed on each evaluation of the element.
A possible improvement over this would be an algorithm that keeps track of the contact points and only considers what has changed since the last evaluation.  
...
Let's denote all points that are part of the convex hull by the index~$k$.
The distance between two surface points~$i$ and~$j$ is given by

\begin{equation}
l_{ij} = \sqrt{(\bar{x}_j - \bar{x}_i)^2 + (\bar{y}_j - \bar{y}_i)^2}
\end{equation}

The total length of the string is the sum of all line segments of the convex hull,
...
The virtual work of the internal forces is obtained by integrating the product of stress~$\sigma$ and the strain variation~$\delta \varepsilon$ over the element's volume, leading to
...
where~$k = EA/l_{0}$ and~$d = \eta A/l_{0}$ are the stiffness and damping, respectively, corresponding to a linear spring-damper, and~$N$ is the normal force within the string.
...
In dynamic analysis, however, especially in the slightly chaotic phase after the arrow leaves the string, it can happen that the string becomes shorter than its initial length, which means that it is under compression.
This must not generate forces that counteract that compression; otherwise the string would behave like a bar and not like a string.
In other words, the stiffness of the string in compression should be zero.
...
We make no attempt to derive a consistent mass matrix for this element, or any mass matrix at all.
...
The idea is, as with the limb, to prescribe a certain damping ratio for the string and identify the viscosity based on that.
But unlike for the limbs, we don't need a numerical modal analysis of the string to do this, since we can carry it out analytically.
